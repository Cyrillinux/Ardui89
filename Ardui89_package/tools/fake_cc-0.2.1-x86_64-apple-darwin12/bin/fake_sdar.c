...
#if defined(_WIN32) || defined(_WIN64)
    sprintf(cmdbuffer, "cmd /c %s\\..\\..\\..\\sdcc\\%s\\bin\\sdar.exe rcs %s\\libraries.a.lib %s", sdccpath, _SDCC_VERSION_, libpath, objslist);
#elif defined(__APPLE__) || defined(__MACH__) || defined(__linux__)
    sprintf(cmdbuffer, "%s/../../../sdcc/%s/bin/sdar rcs  %s/libraries.a.lib %s", sdccpath, _SDCC_VERSION_, libpath, objslist);
#else
#error "OS not supported"
#endif
    system(cmdbuffer);
#if defined(_WIN32) || defined(_WIN64)
                sprintf(cmdbuffer, "cmd /c %s\\..\\..\\..\\sdcc\\%s\\bin\\sdar.exe %s", sdccpath, _SDCC_VERSION_, argsbuffer);
#elif defined(__APPLE__) || defined(__MACH__) || defined(__linux__)
                sprintf(cmdbuffer, "%s/../../../sdcc/%s/bin/sdar %s", sdccpath, _SDCC_VERSION_, argsbuffer);
#else
#error "OS not supported"
#endif
    system(cmdbuffer);
