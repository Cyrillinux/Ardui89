...
int main(int argc, char *argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (argv[i][0]=='-') {
            ...
            strncpy(cc_parameters.tmpfile, cc_parameters.inofilename, strlen(cc_parameters.inofilename)-2);
            cc_parameters.tmpfile[strlen(cc_parameters.inofilename)-2]=0;
            sprintf(argsbuffer, "%s %s.rel", argsbuffer, cc_parameters.tmpfile);
#if defined(_WIN32) || defined(_WIN64)
            sprintf(cmdbuffer, "cmd /c copy %s %s.rel /y", cc_parameters.inofilename, cc_parameters.tmpfile);
#elif defined(__APPLE__) || defined(__MACH__) || defined(__linux__)
            sprintf(cmdbuffer, "cp -f %s %s.rel",cc_parameters.inofilename, cc_parameters.tmpfile);
#else
#error "OS not supported"
            ...
            strncpy(cc_parameters.tmpfile, cc_parameters.currentfilename, strlen(cc_parameters.currentfilename)-2);
            cc_parameters.tmpfile[strlen(cc_parameters.currentfilename)-2]=0;
            sprintf(argsbuffer, "%s %s.rel", argsbuffer, cc_parameters.tmpfile);
#if defined(_WIN32) || defined(_WIN64)
            sprintf(cmdbuffer, "cmd /c copy %s %s.rel /y", cc_parameters.currentfilename, cc_parameters.tmpfile);
#elif defined(__APPLE__) || defined(__MACH__) || defined(__linux__)
            sprintf(cmdbuffer, "cp -f %s %s.rel", cc_parameters.currentfilename, cc_parameters.tmpfile);
#else
#error "OS not supported"
#endif
            system(cmdbuffer);
...
*/
    ...
    sprintf(argsbuffer, "%s %s/../core/core.a.lib", argsbuffer, cc_parameters.sketchpath);
...
/*
#if defined(_WIN32) || defined(_WIN64)
        sprintf(cmdbuffer,"start cmd /k echo \"BEGIN:FAKE_LD:::ORG=%s:::SDCC=%s/sdcc %s:::END;exit\"", orgargsbuffer, cc_parameters.sdccpath, argsbuffer);
        system(cmdbuffer);
#elif defined(__APPLE__) || defined(__MACH__) || defined(__linux__)
...
*/
    // CALL COMPILER
    printf("BEBUG (fake_ld %s) : %s %s/../core/core.a.lib", _FAKE_VERSION_, argsbuffer, cc_parameters.sketchpath);
#if defined(_WIN32) || defined(_WIN64)
