...
#error "OS not supported"
            ...
            sprintf(cmdbuffer, "echo \"void main(void) {\n#ifdef _89C53_\n_stc89_variant_setup();\n#endif\nsetup();while (1) loop();}\" >>  %s/../%s.c",cc_parameters.sketchpath, basename(argv[i]));
#elif defined(_WIN32) || defined(_WIN64)
            sprintf(cmdbuffer, "cmd /c echo \"void main(void) {\n#ifdef _89C53_\n_stc89_variant_setup();\n#endif\nsetup();while (1) loop();}\" >>  %s\\..\\%s.c",cc_parameters.sketchpath, basename(argv[i]));
#else
#error "OS not supported"
#endif
            err=system(cmdbuffer);
            if (err != 0) { // -1 means an error with the call itself
                int err = WEXITSTATUS(err);
                perror("fake_cpp create main in '.cpp' Error !");
            }
#if defined(__APPLE__) || defined(__MACH__) || defined(__linux__)
