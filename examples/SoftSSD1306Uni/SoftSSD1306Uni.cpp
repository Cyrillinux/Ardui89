...
void SSD1306_writeChar(SoftI2cMaster *wire, unsigned char c) {
    unsigned char i;
    if ((c < 32) || (c > 127)) c = 32; // On remplace par espace si hors table
    
    for (i = 0; i < 8; i++) {
        // Lecture directe en Flash sans memcpy_P
        SSD1306_sendData(wire, font8x8[c - 32][i]);
    }
    SSD1306_sendData(wire, 0x00);
}
#else
